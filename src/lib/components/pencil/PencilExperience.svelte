  ...
  <div class="max-w-5xl mx-auto px-6 relative z-10">
    {#if mounted}
      <div in:fly="{{ y: 30, duration: 600 }}" class="mb-16">
        <h2 class="font-display text-5xl md:text-6xl text-graphite-900 mb-2">Experience</h2>
      ...
      <div class="space-y-0">
        {#each experiences as exp, index}
          <div
            class="relative pl-12 py-8 border-l-2 border-graphite-300 hover:border-graphite-900 transition-colors duration-300"
            in:fly="{{ y: 30, duration: 600, delay: 200 + (index * 150) }}"
          >
            <div class="absolute left-0 top-10 -translate-x-1/2 w-4 h-4 bg-graphite-900 rounded-full border-4 border-paper-alt"></div>
            ...
            <div class="group">
              ...
              <h3 class="font-display text-2xl md:text-3xl text-graphite-900 group-hover:text-graphite-700 transition-colors">
                {exp.role}
              </h3>

              <p class="font-handwriting text-lg text-graphite-600 mb-2">
                {exp.company} · {exp.location}
              </p>

              <p class="font-body text-graphite-600 mb-4">
                {exp.description}
              </p>
              ...
              <ul class="space-y-2">
                {#each exp.achievements as achievement}
                  <li class="flex items-start gap-2 text-graphite-600">
                    <span class="mt-2 w-1.5 h-1.5 bg-graphite-400 rounded-full shrink-0"></span>
                    <span class="font-handwriting text-sm">{achievement}</span>
                  </li>
                {/each}
              </ul>
            </div>
